...
const getFreelancerById = asyncHandler(async (req, res) => {
    const freelancer = await User.findById({ _id: req.params.id, role: "freelancer" }).select("-password");
    if(!freelancer) {
...
const updateFreelancerProfile = asyncHandler(async (req, res) => {
    ...
    const updates = {
        ...
        portfolioLinks: req.body.portfolioLinks || freelancer.freelancerProfile.portfolioLinks,
        availability: req.body.availability || freelancer.freelancerProfile.availability,
    }
